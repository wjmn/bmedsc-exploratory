...
[2019-07-08 Mon 13:03]

I'm going to test the exact same configuration, just using a distorted
polar grid instead of a regular polar grid. 

| Encoder learning rate | 1e-4 | 
| Decoder learning rate | 1e-4 | 

I have to remind myself to be patient...and that it is unlikely I'll
see any results for quite a few epochs. I feel like I've been stopping
some of these runs prematurely. 

[2019-07-08 Mon 13:36]
The digits seem extremely dim on the test ones I'm looking at
now...Encoder loss hovers around 5-8.

At epoch 8, the results are not particularly encouraging...

[2019-07-08 Mon 13:52]
After 10 epochs, everything's pretty much disappeared. I'm going to a
try a learning rate of 1e-3 for the encoder and see what happens.

| Encoder learning rate | 1e-3 | 
| Decoder learning rate | 1e-4 | 

This particular grid is not quite as distorted as the previous. We'll
see how it goes. 

[2019-07-08 Mon 14:17]
The evolution seems unstable - I'm going to lower the rate back down
to 1e-4. I can't help but think the success is guided by the specific
...
At Epoch 6 - the digits are extremely dim. I wonder if part of that is
because of the random seed vector. I guess I'll find out when I decide
to stop it. 

Epoch 7 just came in - the digit forms are starting to look a bit
better, but still nothing to write home about. Encoder loss is still
hovering about the 4-6 range.

I think I may try to lower both the gridSize and renderSize for the
next runs. The time for an epoch being 5 minutes at the moment is a
bit excessive...

I'm going to stop it after 8 epochs. There is no noticable multimodal
effect and the output is quite poor...

[2019-07-08 Mon 15:00]

| Grid | Polar Regular Grid |
| Vector size | 10 x 10 = 100 | 
| Render size | 48 x 48 |
| Encoder learning rate | 1e-4 | 
| Decoder learning rate | 1e-4 | 

This is running significantly faster (about 1.5 minutes per
epoch). The output is likely to be poorer though...

[2019-07-08 Mon 15:11]
It's interesting - the encoder loss is hovering around 4-5 but the
output is really poor, at least from what I can see. The decoder loss
hovers around 0.5 to 1... 

I've stopped it at epoch 12 - the outputs are poor, unfortunately. It
didn't really seem to be changing beyond that. It seems the multimodal
effect was sort of evident, but again only in brightness - I wonder if
this is because of the fact that the output vector is linked in a
rather complicated way to the output image via the renderer. 

I'm just going to try this again, but remove the dense hidden layer in
the encoder - so there will only be one dense layer involved i.e. the
output layer. I think this will completely remove any multimodal
effect beyond brightness because there won't be any capability for the
network to abstract from the noise, but we'll see. 

It seems to be evolving quite slowly - observationally, it seems that
having more layers to the encoder increases how fast it trains. So
having few layers causes the encoder to train very slowly.  I'm going
to increase both the encoder and decoder learning rates to 1e-3. 
...
At epoch 8 - results are poor, unfortunately.

[2019-07-08 Mon 15:56]

I'm going to try reducing the base size slightly to accommodate for
the grid (by changing the =a= paramater for the size for grid
generation). I think the grid at the moment is very blurry.

I'm going to try something difference - readding back the two hidden
layers and concatenating with the labels at each step.
...
[2019-07-08 Mon 16:32]
It took a while, but, for some digits, the output looks quite clean at
epoch 21 (notably 2, 3, and 8). The other digits have recognisable
forms but are fairly dim. I'll leave it running for longer and see how
it fares. Decoder loss hovers around 1 and encoder loss hovers
around 3. 

I've stopped it around 22 epochs. Unfortunately, I don't see any
multimodal effects. Perhaps that's an effect of propogating the
conditional label in each layer. I'm going to remove that propogation
(but keep the two hidden layers) and see how it goes). 

[2019-07-09 Tue 10:20]
The results aren't terribly good, but you can make out some of the
digit forms. They seem very unstable. 

Evolution:

[[./03-psychophysics/data/training-intermediate-data/training-gifs/2019-07-08_16-34_evolution_PolarGrid_10-10_48-48.gif]]
...
I'm going to run a trial today. 

Checks
- [X] Chin rest
- [X] Display settings (100% brightness and 100% contrast) 
- [X] Explanatory statement and consent form
- [X] Test example using the cartesian grid (for few full runs)
  
To explain, I'll do:

#+BEGIN_SRC 
python digits.py --grid cartesian --testing
#+END_SRC

I'm going to run the irregular grid I had some time prior:

#+BEGIN_SRC 
python digits.py --ncues 25 --ntrials 20 --grid .\data\training-intermediate-data\training-grids\2019-07-01_10-15_grid_DistortedPolarGrid_12-12_64-64.pkl --encoder .\data\training-intermediate-data\training-encoders\2019-07-01_10-15_encoder_DistortedPolarGrid_12-12_64-64.h5 --processor net
#+END_SRC

And for the brightness-based decoder:

#+BEGIN_SRC 
python digits.py --ncues 25 --ntrials 20 --grid .\data\training-intermediate-data\training-grids\2019-07-01_10-15_grid_DistortedPolarGrid_12-12_64-64.pkl
#+END_SRC

I'm doing 20 trials of 25 digits instead of 25 trials of 20 digits - I
think it's easier to do it during a train. 

*** Analysing the Trial Run

I've just quickly done an individual analysis for the test run:

Here are the plots:

[[file:./03-psychophysics/data/archive/2019-07-09-mg-test-run-mean-accuracy.png]]

[[file:./03-psychophysics/data/archive/2019-07-09-mg-test-run-mean-time.png]]

[[file:./03-psychophysics/data/archive/2019-07-09-mg-test-confusion.png]]

[[file:./03-psychophysics/data/archive/2019-07-09-mg-test-run-rolling.png]]

Results aren't very encouraging, though I can understand why the net
decoder might be so difficult. For reference, here are the images again:

The net decoder:

[[file:./03-psychophysics/data/archive/2019-07-01-generated-images-epoch-6-half-dropout.png]]

The direct brightness comparison:

[[file:./03-psychophysics/data/archive/2019-07-01-comparison-distorted.png]]

Notably:

1. There is a significant lack of contrast in the net decoder, which
   at small sizes is not quite so visible, but when blown up on a
   large screen, becomes quite off-putting. 
2. The comparison/brightness-based digits are denser and have sharper
   forms/edges. 
...
In the background, I am running an 8x8 grid (48x48 render size) with
the seeded encoder and and the categorical decoder with learning rates
of 1e-4 for both encoder and decoder and two hidden layers in the
encoder. 
...
1. Detached the network from the digit decoder for faster training 
2. Changed the encoder architecture to allow a noise seed (currently
   32 random float vector)
3. Tested a conditioned binary-output decoder
4. Added several normalisation and activation layers to the encoder
   architecture
5. Tested one of last week's grids + encoder in a psychophysics
   session 

Some observations:

1. The categorical output decoder seems to fare better than the binary
   output decoder. I suppose there is also a difference in what each
   is trying to optimise /for/ - for example, the binary output
   decoder is trying to optimise for indistinguishability between the
   renders and the MNIST digits, but perhaps this goal isn't necessary
   to optimise for. There is no real need for the digits to be
   indistinguishable (and for some grids, this is impossible), so
   perhaps the categorical output decoder more closely reflects the
   actual desired goal. But following this logic, it would seem that
   there shouldn't be an extra garbage class at all - but this stops
   the digits from "improving" after a certain point. Difficult to
   achieve a balance - how to define what the decoder would see as
   "acceptable"/threshold? 
2. At the moment, I have only tested grids in psychophysics which people
   are able to get to >90% accuracy with - I need to test grids for
   which this is not the case, so that there is actually a
   justification for a different decoder. There is an obvious
   solution - namely, the rescaling grid - as well as simply reducing
   the resolution of the renders (so far, they have all been
   144-phosphene grids). Need to be careful with dropping the render
   resolution as this also changes the ability of the network to train
   since the MNIST digits are fairly thin. I have implemented a few
   mechanisms to try and counteract that, including dropout + blur to
   emulate a phosphene-like appearance, but again there needs to be a
   balance, otherwise the encoder will purposely optimise for a worse
   appearance. 
3. It's possible the MNIST dataset is not suitable for small grid
   sizes, considering how thin the digits are. I think I should
   experiment with further datasets, including a regular digit dataset
   (of similar calibre to that which I am testing for the brightness
   decoder).
4. Contrast and brightness is an issue with the net decoder. I was
   able to get multimodal brightness from the noise vector in one
   experiment, which I think was an interesting feature to have
   learned, but perhaps there needs to be some more specific tuning
   for the loss function. 
   

*** Reading

I'm going to spend a bit of time reading today and consolidating my
knowledge to think about how to approach this problem. 

- GANs, early paper: https://arxiv.org/pdf/1406.2661.pdf
  
*** Binary Rendering

I wonder what will hapen if I enforce a rounding step before the
renders, so phosphenes can /only/ be on or off.

I'm going to try it on an 8x8 polar regular grid at 1e-4 for both
encoder and decoder.

[2019-07-10 Wed 16:34]
The 8x8 polar regular grid didn't perform well after 4 epochs (though
I didn't think it would get any better - changes were highly
incremental). I'm going to try the 12x12 grid with a 1e-3 learning
rate for both encoder and decoder at a render size of 64x64.

[2019-07-11 Thu 09:59]
Unsuccessful - it changed very little during the entire period. 

** [2019-07-11 Thu]

*** Polar Rerun 

I'm running the polar grid at the moment (10x10, 48x48 renders, 1e-3 ) for
the meeting today Zongyuan. It seems that having two hidden layers is
detrimental to the network performance (i.e. it appears that mode
collapse is likely to occur between digits, though on a run with 1e-3
as the learning rate for both encoder and decoder, the 0 and 2 at
least were different and recognisable). 


*** Note to self

Paper to read regarding effect of distortion on object recognition: 
https://onlinelibrary.wiley.com/doi/abs/10.1111/aor.12174

** [2019-07-12 Fri]

*** Meeting Notes and Discussion Points from ZYG Meet

From discussion:

1. Domain classification loss + real/fake binary loss as two separate
   streams (referring to StarGAN https://arxiv.org/pdf/1711.09020.pdf)
   -> I have previously done a domain classification loss + real/fake
   loss as a single output (i.e. 10 classes + collected "garbage"
   renders), but this technically has less output classes than a 10
   class * binary output (i.e. 11 vs 20). Potentially could split off
   the "garbage" into different levels of garbage (which might not
   actually be garbage because the decoder is always just too good),
   though I'm not sure about this quite yet. 

2. Addressing mode collapse - potentially an issue with strong
   training gradients, particularly at the start of training (so all
   modes get led to a single mode early on, and once gradients reduce,
   they travel in the same direction). Recommended WassersteinGAN
   (https://arxiv.org/pdf/1701.07875.pdf) as a reference on the effect
   of different probability distribution difference measures on the
   training step, particularly in regard to gradients (and with
   particular reference to Earth Mover's Distance). Training for what
   I've tried so far happens pretty fast, so also mentioned possible
   to need to tune hyperparameters. 

3. Post-processing/hole-filling to compensate for renders being
   imperfect. Ideally the post-processing/hole-filling step would be
   comparable to inferences that people can make about edges in an
   imperfect, holey image. It could be a GAN (e.g. CycleGAN
   https://arxiv.org/pdf/1703.10593.pdf), but I imagine something less
   sophisticated could do the job too. An easy training set is to
   train against the MNIST digits with the holes + gaussian blur I've
   put in vs the ground truth MNIST (i.e. going from "holes" to real
   images), which could be applied on post-processing to the simulated
   renders before giving it to the decoder. 

4. Early stopping - probably not too big of a deal, just need to
   monitor the GAN. There are some metrics which could probably do
   this more quantitatively (e.g. when loss stops descending according
   to some parameter).

I spent some considerable time reading into the StarGAN paper and
WassersteinGAN paper. It's very interesting to see how they've
approached these problems.

** [2019-07-15 Mon]

*** Hole-Filling Network

I've tried to implement a hole-filling network based loosely on the
CycleGAN architecture (https://arxiv.org/pdf/1703.10593.pdf), although
without the cycle part as I don't yet think that's relevant for this
particular problem (I think you could use the grid to do the
translation bidirectionally, but I have to think about that more. If I
did do that, I think it would be a bit too similar to masking...)

Training is /very/ slow, at least compared to all the networks I've
been training before. Probably because of all the layers I've put in
the encoder (and the decoder too). I'm not sure all the layers are
strictly necessary, but worth I try I think. 

I would estimate it's taking about two to three times as long as prior
training, which would make it about...20-30 minutes per epoch? :( 

On the plus side (maybe?), from my shallow observations of the
training losses, it does seem much more "push and pull" in that the
encoder and decoder losses rise and fall more nicely (though that's
obviously because now the encoder has full access to the image
pixels). 

[2019-07-15 Mon 11:50]
First epoch is done...the forms look simultaneously gritty and vague,
but they have discrete blob like shapes. 

Although...I have actually just realised I did not incorporate any
class requirements on this generative adversarial network i.e. that
they should map to the same digit. Hm. 

[2019-07-15 Mon 13:04]
After 4 epochs...not too shabby for a completely naive first
run. There are definitely digit forms appearing, and they don't always
correspond to the digit that was the original (e.g. the 0 becomes a 6
for a certain dropout) - which I think is actually /good/ because that
simply means the dropout digit was a poor representation of the actual
digit (at least, I am led to believe so). 

Training does take 20 minutes per epoch though...which is a shame.

[2019-07-15 Mon 13:14]
...or, maybe scratch that thought. Epoch 5 looks cleaner, but the
digits look much weirder. I would hazard to say that the
transformation is doing a bit /too/ much (really, I just want it to
connect the blobs into clean lines, not transform the whole thing). 

Decoder loss hovers around 1.4ish and encoder loss hovers around
0.6ish. I guess it's not terribly surprising given that the decoder is
the sum of two binary cross entropies of the dataset (the real and the
fake) whereas the encoder is only one.


[2019-07-15 Mon 13:59]
Epoch 7...it seems to be getting /worse/. That's very unfortunate. 

[2019-07-15 Mon 14:22]
I've stopped it at Epoch 8 because I think it's not changing much
anymore - it appears quite stagnant. 

Here's the loss curve, much more "adversarial" push and pull looking
than previous loss curves. 

[[file:./03-psychophysics/data/training-hole-filling/training-graphs/2019-07-15_11-16_loss_48_48.png]]

And here's the training gif. 

[[file:./03-psychophysics/data/training-hole-filling/training-gifs/2019-07-15_11-16_evolution_48_48.gif]]

I think the appearance looks best at about epoch 4-5, after which it
seems to get worse. 

I'm going to try and simplify the networks a bit which should 1)
hopefully make the training quicker, and 2) might prevent the weird
changes. I'm also going to reduce the epoch size by 3 times (to 10,000
samples) so I can see the progression faster.  

Just for reference, these are the models I just tried:
...
Both the encoder and decoder had learning rates of 1e-4. 

[2019-07-15 Mon 14:30]

Here are the models I'm going to try next. I've removed two of the
encoder layers and one of the decoder layers.
...
I have reduced the number of samples to 10,000 per epoch. I'll keep
the learning rates at 1e-4. 

[2019-07-15 Mon 14:59]

Training time is shorter - about 5-6 minutes per epoch. Don't know
about the results though.

[2019-07-15 Mon 15:00]

Epoch 5 - it's worse in comparison to the epoch 5 of last run, but
then it hasn't run through a comparable amount of training images
yet...Still, not sure if I should just stop this. The decoder and
encoder are still going up and down, so maybe I'll keep it going. 

[2019-07-15 Mon 15:26]

It's bounding around a lot - each alternate epoch it is "bright" then
"dark." I'm going to add back one of the convolutional layers to the
encoder, then increase the amount of data per epoch to 20,000...Also
changed the plotting, so it's in a strip of values rather than the 2x5
grid (meaningless for this). 

I think the next step to try after this is possibly the StarGAN
approach or WGAN approach. 


I'm not sure why the decoder seems to have a significantly poorer
discrimination accuracy than before - it's getting about a 1.2ish to
1.3ish consistently, which seems strange ( I would have thought the
decoder would be much better at distinguishing real images from fake
images...). 

I wonder if potentially the activation function or learning rate could
be better optimised, but I think I should just let GAN run for a while
before jumping to any conclusions about what needs to be done.

[2019-07-15 Mon 16:44]
I'm going to run this overnight with all the data...I got rid of one
of the downsampling convolution layers and replaced it with a stride1 layer.

** [2019-07-17 Wed]

*** Hole-Filling Progress

With all the data, and the following architecture:
...
It takes abou 50 minutes per epoch to run. 

Here's the original MNIST digits:

[[file:./03-psychophysics/data/archive/2019-07-17-mnist-original-digits.png]]

And with dropout:

[[file:./03-psychophysics/data/archive/2019-07-17-mnist-digits-with-dropout.png]]

And at epoch 14:

[[file:./03-psychophysics/data/archive/2019-07-16-mnist-epoch-14.png]]

And at epoch 47:

[[file:./03-psychophysics/data/archive/2019-07-17-mnist-epoch-47.png]]

Just going to keep this running at epoch 50. 

*** Once the Hole-Filling Completes...

I will next try to run the hole-filling encoder as part of the
training, and maybe also try the binary-fake and categorical decoders
as separate decoders. 

*** Hole-Filling Completed

Here's the loss curve. I think this suggests that there encoder needs
to be stopped earlier, but at least it's obvious now...

[[file:./03-psychophysics/data/training-hole-filling/training-graphs/2019-07-15_16-47_loss_48_48.png]]

And here's the training gif:

[[file:./03-psychophysics/data/training-hole-filling/training-gifs/2019-07-15_16-47_evolution_48_48.gif]]

*** Running with Hole-Filling and Double Decoder

I'm going to try running this on Massive.

I've cloned the repo into =fs11_scratch/jamin/bmedsc-exploratory=. I'm
using light compute at the moment, though it's only going to give me
four hours. 

First I need to load the modules. Looks like I need to run this:

#+BEGIN_SRC 
module load anaconda/5.1.0-Python3.6-gcc5
#+END_SRC

I created a conda virtual environment.

#+BEGIN_SRC 
conda create --name bmedsc-exploratory
#+END_SRC
...
=pip install requirements.txt= didn't work. So I might just try to do
a minimal installation...

I confirmed that pip was indeed using the =conda=-installed pip with
=which pip=. 
...
#+BEGIN_SRC 
pip install opencv-python Pillow numpy pandas scipy matplotlib
#+END_SRC
...
Then, because tensorflow wasn't found, 

#+BEGIN_SRC 
pip install tensorflow==2.0.0b1
#+END_SRC

It occured to me that I am maybe not using tensorflow-gpu...

But it's still not working. It simply fails with =module tensorflow
not found=  from the Jupyter notebook, and doing it from the shell
shows a missing 

#+BEGIN_SRC 
ImportError: /lib64/libm.so.6: version `GLIBC_2.23' not found (required by /home/wjm07/.conda/envs/bmedsc-exploratory/lib/python3.7/site-packages/tensorflow/python/_pywrap_tensorflow_internal.so)
#+END_SRC

I'll try running this instead:

#+BEGIN_SRC 
pip install tensorflow-gpu==2.0.0-beta1
#+END_SRC

I also just decided to run 

#+BEGIN_SRC 
conda install jupyter
#+END_SRC

Just in case Jupyter wasn't actually running the activated environment...


..It took a while. And unfortunately, I'm still getting the GLIBC_2.23
not found error. 

I'll just run it on the local machine here...

So I've set it up. It's quite slow...I'm using 1e-4 for the learning
rate for each.

Here are the architectures for each:
...
The filler network is in between:
...
Maybe I'm just impatient, but the encoder's been hovering around 18
for a while. I'm going to up the learning rate I think. 

I've upped the learning rate of the encoder to 5e-4 and keeping the
decoder learning rate to 1e-4. I'm going to take out the hidden layer
for the encoder too.

[2019-07-17 Wed 14:37]
The encoder also seems to hover around 16-17...

I wonder if I should change less for this run - e.g. maybe keeping the
loss the same?

Though the loss does seem to gradually be decreasing, albeit at a low
rate. 

[2019-07-17 Wed 14:55]
Loss has dropped down a little bit to 15-16. That's good!
...
[[file:./03-psychophysics/data/archive/2019-06-18-generated-digits-epoch-50-filled-first-attempt.png]]

The filled digits:

[[file:./03-psychophysics/data/archive/2019-06-18-generated-digits-with-fill-first-attempt.png]]

And the loss curves:

[[file:./03-psychophysics/data/training-intermediate-data/training-graphs/2019-07-17_14-20_loss_DistortedPolarGrid_12-12_48-48.png]]

The encoder loss just /wouldn't go down/ past 15. That's very
frustrating, I'm not entirely sure why. Perhaps I shouldn't have tried
to change so much in one go - i.e. I probably should have left the
actual loss the same. 

Maybe I should prioritise the "this is fake" decoding more?

Interestingly, the decoder /can/ decode the digits from the filled
render, and it knows all of them are fake. But they really should all
be garbage. Maybe in this particular paradigm, I should not do the two
branches separately - i.e. it shouldn't give a label to garbage!

*** New Run -> Back to the 11-class decoder

I'm going to rerun this with the 11-class decoder. Learning rate of
1e-4 for both encoder and decoder.

[2019-07-18 Thu 12:01]
Loss went up to 10 for the encoder, now is hovering around 8 after 4-5
epochs. Decoder loss is below 1 (hovering around 0.2). 

[2019-07-18 Thu 13:06]
At 7 epochs...it's still hovering around 8. It's changing exceedingly
slowly. I'm going to stop this run. Results are poor.

*** Changes to =phosphenes.py=

While the new run is running, I made a number of simplifications to
=phosphenes.py= namely, making sure that the electrode sizes are tied
to the location and changing "distorted" to "random" (which is what it
is - a random map of phosphenes). 

*** Running Hole-Filling Again

I'm going to run the hole-filling again - this time, testing on
"actual" grid renders (albeit for the polar grid). I can't justify
training on the random/distorted grid because then it would literatlly
just unmask the image (which begs the question, why not just generate
MNIST digits and then mask them for testing? because the decoder can
learn a level of inference beyond what a human can learn. Maybe). 

I'll only go for 20 epochs this time. 

(In hindsight, perhaps it would be better to do multiple random
grids - for example, using just the polar grid might not cover the
(constant) holes in where the grid does not reach.

So I've done that - each /batch/ makes a new grid. I've also reduced the
number of encoder kernels (from 64 to 48, from 128 to 96 and from 256
to 128 at each layer) which will hopefully make both training and
forward passes faster. 

[2019-07-19 Fri 09:54]

After 20 epochs, it happened to end on a poor image. It's /just/ at
the crossroads where decoder loss has now gone below encoder loss. I'm
going to train for 20 more epochs and keep a close eye on it and maybe
stop it earlier. 

*** Using Massive

It's clear I'm not going to be able to use Massive without changing to
the tensorflow 1.14 API. I'm going to do a bit of reorganisation.

** [2019-07-19 Fri]

*** CUDA Version Issues

I just tried to run something with tensorflow on Massive. I got this
error:
...
This is very frustrating. Granted, I'm not using the Massive "module"
version of tensorflow. Maybe I should try that...

I might have to use an even earlier version of tensorflow - it looks
like they have support for 1.12.0...

Couldn't install 1.12.0.

Here's what I'm going to do - instead of using conda, I'm just going
to use the tensorflow module. 

#+BEGIN_SRC 
module load  tensorflow/1.12.0-python3.6-gcc5
#+END_SRC

Then, I've installed a few packages via pip.

#+BEGIN_SRC 
pip install --user opencv-python pillow scikit-image ipykernel jupyter jupyterlab
#+END_SRC

I can't seem to get Jupyter notebook working, but fine, that's
alright. Working from the console seems fine.

[2019-07-19 Fri 13:15]

Actually, I can get Jupyter to work with tensorflow fine - just need
to make sure to use the one I installed with =pip= since it's not on
my path i.e. 
...
*** Hole-Filling (Second Time)

I stopped it after 5 additional epochs (so it's had 25 epochs). 

Evolution:

[[file:./03-psychophysics/data/training-hole-filling/training-gifs/2019-07-18_13-57_evolution_48_48.gif]]

Loss curve:

[[file:./03-psychophysics/data/training-hole-filling/training-graphs/2019-07-18_13-57_loss_48_48.png]]

I wonder if the learning rate (1e-4 for both encoder and decoder) was
still too high for this?.

*** Run with New Hole-Filler

I'm going to try a run with the new hole-filler which is specifically
for 8x8 phosphenes on a 48x48 grid. 

I'm doing a new CGAN run with this new hole filler. 

#+B> Contributed by **moderator**
>
...
[2019-07-22 Mon 09:27]

Results weren't very good, but they were improving. It sort of seems
like running for a few more epochs might be beneficial. 

[2019-07-22 Mon 11:16]

Stopping at 59 epochs. Results aren't good, they're not bright
enough. The forms...it's hard to tell when they're not bright enough. 

Training gif:

[[file:./03-psychophysics/data/training-intermediate-data/training-gifs/2019-07-19_12-46_evolution_RandomPolarGrid_8-8_48-48.gif]]

Loss graph:

[[file:./03-psychophysics/data/training-intermediate-data/training-graphs/2019-07-19_12-46_loss_RandomPolarGrid_8-8_48-48.png]]

Here's the hole-filled final renders of each digit:

[[file:./03-psychophysics/data/archive/2019-07-22-hole-filled-59-epochs.png]]

There weren't any additional modes. 

I'm going to do some more reading I think. I may explicitly need to
incentivise contrast, but my past experiments were not very good at
doing this. 

** [2019-07-22 Mon]

*** Running on Regular Polar Grid

I'm going to run the exact same experiment (though with one hidden
layer) for the regular polar grid while I do my reading. 

Yan's suggested to increase the luminence of the final renders to
check. I think I will do that. 

[2019-07-22 Mon 14:37]

I stopped it at epoch 15. 

Here are some comparisons:

Renders at epoch 15:

[[file:./03-psychophysics/data/archive/2019-07-22-polar-regular-renders-epoch-15.png]]


With hole-filling:

[[file:./03-psychophysics/data/archive/2019-07-22-polar-regular-renders-epoch-15-filled.png]]

With binarised-values for clarity, with a threshold of 0.12 (chosen
manually):

[[file:./03-psychophysics/data/archive/2019-07-22-polar-regular-renders-epoch-15-binarised-values.png]]

(It looks like I really need to incentivise the network to produce
binarised values at the end...)

The binarisation exposes that there are multiple modes produced (at
least for this run), but these do appear only to determine which
phosphenes in the form are lit (not the actual form itself):

[[file:./03-psychophysics/data/archive/2019-07-22-polar-regular-renders-epoch-15-modes-4.png]]

And the loss curve (maybe i should have let it train more..)

[[file:./03-psychophysics/data/training-intermediate-data/training-graphs/2019-07-22_11-42_loss_PolarGrid_8-8_48-48.png]]

*** Running on Irregular Polar Grid

I'm going to run on the irregular polar grid again, and try looking at
the binary output as well.

I've simplified the encoder a bit:
...
[2019-07-23 Tue 09:42]

Finished running 50 epochs. Results are, well similar. 

Phosphene locations:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-locations.png]]

Comparison:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-comparison.png]]

Comparison with filling:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-comparison-filled.png]]


Training gif:

[[file:./03-psychophysics/data/training-intermediate-data/training-gifs/2019-07-22_14-57_evolution_RandomPolarGrid_8-8_48-48.gif]]

Training loss graph:

[[file:./03-psychophysics/data/training-intermediate-data/training-graphs/2019-07-22_14-57_loss_RandomPolarGrid_8-8_48-48.png]]

Net encoder after 50 epochs of training:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-epoch-50.png]]


Net encoder with hole-filling:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-hole-filled.png]]


Net encoder, brightened with threshold of 0.122 selected manually:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-epoch-50-brightened.png]]

Evidence of multimodal 8, apparent on threshold binarisation:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-multimodal-8.png]]

And, out of interest, here is the (non-brightened) output with a seed
vector full of zeros:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-zero-seed.png]]

And a seed vector full of ones:

[[file:./03-psychophysics/data/archive/2019-07-23-random-polar-grid-one-seed.png]]

It seems the seed vector encodes for contrast perhaps. 

All in all - it's clear the network learns /something/, and the
brightening step sort of reveals the features it's getting out. But
there are a lot of distracting elements (e.g. that light at the top
which does nothing!) that could be removed. 

I just wish these would be brighter...

**[2019-07-23 Tue]
    
*** Run without noise seed and brightness incentive

I will try removing the noise vector seed entirely, and putting an
incentive for the phosphene values to be either 0 or 1. I'll also use
a different random polar grid. I'll also increase the learning rate to
1e-3 for the encoder and decoder.
...
I've redefined the loss of the encoder to
...
[2019-07-23 Tue 11:59]

1e-3 bounces around a lot. I'm going to try 1e-4 instead.

[2019-07-23 Tue 13:08]

1e-4 is /extremely/ slow (epoch 5 and no difference really)...I'll try
1e-4 for the decoder and 1e-3 for the encoder.

** [2019-07-25 Thu]

*** Non-multimodal run (one-to-one label)

Here are the results of the 1-1 run over 50 epochs: 

[[file:./03-psychophysics/data/archive/2019-07-25-epoch-50-random-non-multimodal.png]]

Hole filled:

[[file:./03-psychophysics/data/archive/2019-07-25-epoch-50-random-non-multimodal-hole-filled.png]]

Brightened at manual threshold of 0.124:

[[file:./03-psychophysics/data/archive/2019-07-25-epoch-50-random-non-multimodal-brightened.png]]

For comparison, here are the directly processed digits:

[[file:./03-psychophysics/data/archive/2019-07-25-random-non-multimodal-comparison.png]]

Loss curves:

[[file:./03-psychophysics/data/training-intermediate-data/training-graphs/2019-07-23_10-22_loss_RandomPolarGrid_8-8_48-48.png]]

It seems the digit forms are more stable than before at the very
least. It definitely seems like the output is somewhat "cleaner", but
I wonder why it's not trying to brighten the pixels. I didn't make the
contribution of the cosine function particularly large, though I don't
know if making it larger would necessarily help. 
...
- =scancel=
- =show_job=
- =smux new-session=, and =smux a= to attach or Ctrl-b d to detach
- =squeue= with =grep=
- =module load tensorflow/2.0.0-beta1=

** [2019-07-30 Tue]

*** Modifications for Massive

I made a few modifications to the script for massive and saved it in 04-gan.

I'm going to try and make a proper venv.

Loaded tensorflow. Curled pip from https://pip.pypa.io/en/stable/installing/ and installed for user. 

Pip installed to =~/.local/lib/python3.6/site-packages/pip/=, but can't use the bin in =~/.local/bin/pip=
directly.

But =python -m pip install PACKAGE --user= works!

- python-opencv 4.1.0 throws an import error for libSM.so.6, but python -m pip install opencv-python==3.3.0.9 --user works

Ran
#+BEGIN_SRC 
python -m pip install --user pillow scikit-image numpy scipy pandas matplotlib
#+END_SRC

It seems to be running find on Massive now. It doesn't seem all that
much faster (though granted, I didn't provision it with particularly
many resources or a GPU, but at least I can run several at the same
time and do other prototyping on this computer. 

It turns out that 1:00:00 means one hour, so my job got
cancelled. I'll rerun with 48:00:00 and provision 2 CPUS and see if
that's faster. I'm also going to run it with 1 GPU, and see how that goes...

Seems to be running quite a bit faster (at least 3 times). That's good
at least.
...
*** Minor Fixes + MASSIVE run
:LOGBOOK:
CLOCK: [2019-08-01 Thu 11:16]--[2019-08-01 Thu 12:44] =>  1:28
:END:

I'm going to run a few more runs on MASSIVE I think. They seem to work alright,
I just needed to fix up an image directory.

I submitted Job =10702348= - same as before, ( I think I'll have to think about
what I actually want to loop through before I submit a huge number of jobs for
something ). In the meantime, I'll get to work on the grid evaluation and the
binarisation.

While I'm waiting for it, I've written the code to do iterate through the
thresholding and also to do the rendering and preprocessing for a given grid.

*** Second Run Finished + Testing Threshold Iterator
:LOGBOOK:
CLOCK: [2019-08-01 Thu 14:17]--[2019-08-01 Thu 15:15] =>  0:58
:END:

Second run has finished - now testing the threshold iterator.

** [2019-08-19 Mon]

I wrote a script to do combinations of 11 grids x 3 encoder x 2 decoders x 2
filler options (fill or no fill) x 3 learning rates = 396 different
combinations. I submitted all of them to Massive. 396 jobs, 4 at a time (since
they use GPU). I don't know why the job name is jus TEST_CGAN rather than the
whole thing though. That seems a bit strange to me. But at least they're running now....

I've also just realised that the direct encoder is missing a flatten...so all
the runs with that encoder will fail. I guess that's okay, they fail pretty
early on - but that should amount to approximately one third of the submissions
(132), so I'll only have 264 to work off instead. I'll have to keep that in mind
when processing the data.

I think I'll start getting my thesis written and go through the methods section.
I think that will also help clarify my thinking and possibly give me a few more
ideas to test.

** Notes from Post-Experiment Debrief on Psychophysics

- May not be a natural perceptual task - feels 100% like a memory task indeed.
- Learning effect is definitely significant - by the time the second trial comes
  round, participant knew to "play the game", so to speak.
- During the experiment, you have the opportunity to really study the images
  (because they're static), which may not generalise to real-life when these
  images move with your eyes.


** [2019-08-28 Wed]

Generated 10 new 8x8 grids on Massive. Submitted 20 jobs (filler and non-filer
for each grid, basic encoder and basic decoder, 1e-3 learning rate for encoder
and 5e-4 for decoder). Had to do run_jobs with python3 with the python3 module
loaded, not tensorflow module for some reason, but seems to be okay other than
that.

For psychophysics experiment tomorrow:


1. Split each into batches of 400 and interleave -> so 400 for direct, 400 for
   trained, 400 for direct, 400 for trained (or vice versa). So each participant
   gets four trials.
2. Start each with 100 non-feedback trials (50 trials of each grid) WITH NO
   FEEDBACK. Maybe copy your script and make some slight modification.
3. Try to add progress text.

I got the above ready in time, though I had to jump over an obscene number of
hoops. For one thing, there was apparently a change from Python 3.6 to 3.7 with
how StopIteration is handled in generators, so I had to downgrade my Python
installation. For another, I had to reinstall most of the packages, and lo and
behold that many of the packages sort of broke compatibility as they increased
their version, so I had to specifically uninstall and reinstall the following
versions:

- PyQt5==5.9
- pyglet==1.3.0

Also, for Pycharm, needed to remap the network drive to a windows drive with the
=subst= command. I also removed logging for mouse movements, since I'm not using
that at all for the experiment.

This experiment takes quite a bit longer - I think I should block out 1 and a
half hours for it instead.
